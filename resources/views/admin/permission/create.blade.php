            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-10">
                <div class="py-2 px-6">
                    ...
                    <div class="bg-gray-100 px-4 mt-1">
                        @if ($errors->any())
                            <ul class="py-0.5 list-none list-inside text-base text-red-400">
                ...
                <div class="w-full px-6 py-4 bg-white overflow-hidden">
                    <form method="POST" action="{{ route('permission.store') }}">
                        ...
                        <div class="py-2">
                            <label for="name"
                                   class="block font-medium uppercase text-sm text-gray-400{{$errors->has('name') ? ' text-red-400' : ''}}">{{ __('Nome') }}</label>
                            <input id="name"
                                   class="rounded-none shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full{{$errors->has('name') ? ' border-red-400' : ''}}"
                                   type="text"
                                   name="name"
                                   value="{{ old('name') }}"
                            />
