        ...
        <div class="p-0 flex justify-end items-center">
          ...
          <div class="bg-gray-100">{{-- Icona de exportar a excel --}}
            <div class="d-print-none with-border mt-2 py-4 mr-3">
              <a href="{{ route('admin.exportcidadan') }}" class="justify-center">
                <!-- compoñente tooltip -->
                <div class="relative flex inline-flex flex-col items-center group">
          ...
          <div class="bg-gray-100 py-4">{{-- Icona de exportar a pdf --}}
            <div class="d-print-none with-border mt-2 p-0">
              <a href="{{ route('admin.exportpdfcidadan') }}" class="text-red-700 justify-center">
                <!-- compoñente tooltip -->
                <div class="relative flex inline-flex flex-col items-center group">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24"
                    ...
                    <tr>
                      ...
                      <th
                        ...
                        @include('admin.includes.sort-link', [
                            'label' => 'Idade',
                            'attribute' => 'datanacemento',
                        ])
                      </th>
                      @canany(['cidadan edit', 'cidadan delete'])
                        <th colspan="2"
                          class="py-4 pl-6 pr-3 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light text-right">
                          {{ __('Actions') }}
                        </th>
                      @endcanany
                    </tr>
                        ...
                        <td
                          ...
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->nome }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->apel1 }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->nif }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->codpostal }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->email }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->tfno1 }}</a>
                          </div>
                        </td>
                        <td
                          class="border-b border-slate-100 p-1 pl-6 text-slate-500">
                          <div class="text-sm text-gray-900">
                            <a href="{{ route('cidadan.show', $cidadan->id) }}"
                              class="no-underline hover:underline text-cyan-600">{{ $cidadan->datanacemento }}</a>

                            {{-- $edad = Carbon::parse($cidadan->datanacemento)->age; // 1990-10-25
                                                    dump($edad) // 26 --}}
                          </div>
                        </td>
                        @canany(['cidadan edit', 'cidadan delete'])
                          <td
                            class="border-b border-slate-100 p-1 pl-6 text-slate-500 text-right">
