                ...
                <div class="p-2 w-22 h-22 flex justify-end items-center">
                    @can('empresa create')
                            <div class="d-print-none with-border mt-2 p-0">
                                <a href="{{ route('empresa.create') }}" class="text-blue-900 hover:bg-grey-200 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 justify-center">
                                ...
                                <table class="border-collapse table-auto w-full text-sm">
                                    ...
                                    @foreach($empresas as $empresa)
                                        <tr>
                                            ...
                                            <td class="border-b border-slate-100 dark:border-slate-700 p-1 pl-8 text-slate-500 dark:text-slate-400">
                                                <div class="text-sm text-gray-900">
                                                    <a href="{{route('empresa.show', $empresa->id)}}" class="no-underline hover:underline text-cyan-600 dark:text-cyan-400">{{ $empresa->email }}</a>
                                                </div>
                                            </td>
                                            <td class="border-b border-slate-100 dark:border-slate-700 p-1 pl-8 text-slate-500 dark:text-slate-400">
                                                <div class="text-sm text-gray-900">
                                                    <a href="{{route('empresa.show', $empresa->id)}}" class="no-underline hover:underline text-cyan-600 dark:text-cyan-400">{{ $empresa->created_at }}</a>
                                                </div>
                                            </td>
                                            @canany(['empresa edit', 'empresa delete'])
                                                <td class="border-b border-slate-100 dark:border-slate-700 p-1 pl-8 text-slate-500 dark:text-slate-400">
                                                    ...
                                                    <a href="{{route('empresa.show', $empresa->id)}}" class="no-underline hover:underline text-cyan-600 dark:text-cyan-400">
                                                        <!-- compoñente tooltip -->
                                                    ...
                                                    <form action="{{ route('empresa.destroy', $empresa->id) }}" method="POST" class="flex inline-flex">
                                                        @can('permission edit')
                                                            <a href="{{route('empresa.edit', $empresa->id)}}" class="px-2 py-2 text-cyan-600">
                                                                <!-- compoñente tooltip -->
                                                                <div class="relative flex inline-flex flex-col items-center group">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
